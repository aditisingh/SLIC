...
M				    HANDLE_ERROR(cudaMalloc(&d_gpu[0], N*sizeof(double)));
				...
				HANDLE_ERROR(cudaFree(labels_gpu[0]));
				HANDLE_ERROR(cudaFree(labels_gpu[1]));

				HANDLE_ERROR(cudaFree(Pixel_LAB_gpu));
				HANDLE_ERROR(cudaFree(centers_gpu[0]));
				HANDLE_ERROR(cudaFree(centers_gpu[1]));

				HANDLE_ERROR(cudaFree(d_gpu[0]));
				HANDLE_ERROR(cudaFree(d_gpu[1]));
