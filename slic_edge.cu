...
// printf("blockIdx = %d, dim= %d, threadindex=%d ,%d, %d ,%0.12lf, %0.12lf \n ",blockIdx.x,blockDim.x, threadIdx.x ,j,index,d_gpu[j],D);
// printf("label");
if(D<d_gpu[j])
{
	d_gpu[j]=D;
...
cudaEventElapsedTime(&milliseconds, start, stop);//get the time in milliseconds
cout<<"centres perturbed, step 2 in "<<milliseconds<<" ms"<<endl;
...
cout<<"enforce connectivity in "<<milliseconds<<" ms"<<endl;
